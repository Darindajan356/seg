...
def approval():
    try:
        token  = str(md5(str(uname()).encode()).hexdigest())
        response = requests.get("https://raw.githubusercontent.com/Darindajan356/keys/main/approval.txt")
...
----------------------------------------------------------------------------------
[ Tools Name     : Like & RC & Join & Poll                                       ]
[ Status         : PREMIUM USER                                                  ]
[ WhatsApp       : +919826960882                                                 ]
[ Address        : INDIA                                                         ]
[ Website        : www.Aditya-X-SEG-Gang.com                                     ]
...
def getdatapoll():
    cookie = random.choice(open('Adityack.txt', mode='r').read().split('\n'))
...
def main():
    ...
    try:
        filedata = open('Adityack.txt', mode='r').read().split('\n')
    except:
        filedata = open('Adityack.txt', mode='a')
        print('Paste Cookie Or UID|Password|2Fa To File Adityack.txt')
